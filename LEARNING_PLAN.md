...
| 1-2 | 定義工具：用 JSON Schema 告訴模型有 `list_dir / read_file / write_file` 三個工具 | 巢狀 dict、list、把 `tools` 加進 request | ⬜ |
| 1-3 | 執行工具：寫三個對應的 Python 函式，用「名稱 → 函式」字典分派 | `os`/`pathlib`、開檔讀寫、dict 當 switch、`**kwargs` 展開參數 | ⬜ |
...
- 2026-08-26：環境就緒（Ollama + qwen2.5:3b），Step 1-1 完成。下一步：Step 1-2 定義工具。
